    ...
    for row_topics, row_images in zip(topics, images):
      ...
      for i, topic in enumerate(row_topics):
        with cols[i]:
            ...
            col1, col2, col3 = st.columns([1, 2, 1])  # Dividing the column into three parts.
            with col2:  # Only placing the button in the middle column.
                if st.button(topic, key=f"button_{topic}"):
